...
export default function EvaluationPage({ evaluationData, sessionId }) {
    const router = useRouter();
    const metricKeys = Object.keys(RUBRIC_DEFINITIONS);
    
    // New state variables to track saving status
    ...
    useEffect(() => {
        const saveEvaluation = async () => {
            setIsSaving(true); // Show "Saving..." message
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    setSaveStatus('error'); // Show error message
                ...
                setIsSaving(false); // Hide "Saving..." message
            }
        };

        saveEvaluation(); // Call the function
    }, [evaluationData, sessionId]); // Run when evaluationData or sessionId changes
            ...
            <div className="max-w-7xl mx-auto">
                <header className="flex justify-between items-center mb-8">
                    <div>
                        ...
                        {isSaving && (
                            <p className="text-sm text-emerald-600 mt-1">Saving evaluation...</p>
                        )}
                        {saveStatus === 'saved' && (
                            <p className="text-sm text-emerald-600 mt-1">✓ Evaluation saved to database</p>
                        )}
                        {saveStatus === 'error' && (
                            <p className="text-sm text-red-600 mt-1">⚠ Failed to save evaluation</p>
                        )}
                    </div>
                    <button 
                ...
                {/* Keep your existing metrics grid */}
